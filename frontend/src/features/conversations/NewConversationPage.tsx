...
export function NewConversationPage() {
  ...
  return <section className="mx-auto max-w-xl px-5 py-7">
    ...
    <div className="mt-8 rounded-3xl border bg-gloopy-surface p-5 sm:p-7">
      <div className="grid grid-cols-2 rounded-xl bg-gloopy-bg-dark p-1" role="tablist"><button role="tab" aria-selected={mode === 'direct'} onClick={() => changeMode('direct')} className={`min-h-11 rounded-lg font-semibold ${mode === 'direct' ? 'bg-gloopy-primary text-white' : ''}`}>Conversa</button><button role="tab" aria-selected={mode === 'group'} onClick={() => changeMode('group')} className={`min-h-11 rounded-lg font-semibold ${mode === 'group' ? 'bg-gloopy-primary text-white' : ''}`}>Grupo</button></div>
